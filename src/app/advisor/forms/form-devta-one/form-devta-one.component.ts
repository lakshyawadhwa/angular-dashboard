...
@Component({
  selector: "app-form-devta-one",
  templateUrl: "./form-devta-one.component.html",
  styleUrls: ["./form-devta-one.component.scss"],
})
export class FormDevtaOneComponent implements OnInit {
  constructor(
  ...
  headingArray = [
    "Zone",
    "Inner 12 Devta",
    "Evaluation(Dev/ Asur / Polarity / Strength/ Presence of Devta)",
    "Suggestions",
  ];
  query: UserQuery;
  ...
  ngOnInit(): void {
    ...
    this.formService
      .getForm(this.query.queryId, this.siteDetails.siteId, "LEVEL_1_K_DEVTA_1")
      .subscribe((res) => {
  ...
  submitForm() {
    this.responseArray.map((response) => {
      response["level"] = "LEVEL_1_K_DEVTA_1";
